...
**最終更新日時: 2026-08-18 03:07:00(日本時間) / バージョン: 08.18.3**
...
- **身長ベースの距離推定(2026-08-17に追加)が、実際には遠くの人物を系統的に手前へ
  引き寄せてしまう不具合を修正。**
  「以前はとても前後(奥行き方向)で奥まできれいに判定できていたのに、何か変わったのでは」
  というご指摘を受けて調査した。2026-08-17に追加した身長ベースの距離補正
  (`poseGeometry.js`の`imageToFloor()`)は、「決め打ちの想定身長(`ASSUMED_PERSON_HEIGHT_M`
  =1.1m)から求めた距離」と「床平面への投影距離」を毎フレーム平均する実装になっていたが、
  これは実際の身長が1.1mからズレている人(=ほとんどの大人・年長の子供)について、
  **実際の距離に関わらず常に一定の割合で手前に引き寄せてしまう**、という系統的な不具合
  だった(計算式を数値で検証したところ、身長1.7mの大人の場合、距離に関わらず常に約17.6%
  手前に補正されてしまうことを確認した)。想定身長にちょうど近い体格の人物(子供)だけは
  影響が小さく、大人ほど・想定身長からズレるほど、部屋の奥にいても実際より手前寄りにしか
  判定されなくなるため、「奥まできれいに判定できていたのが、できなくなった」ように見えて
  いた。
  - **修正方針:** 「決め打ちの想定身長で平均して引き寄せる」方式をやめ、「大人〜子供、
    どんな身長の人でもさすがにこれより高くはないはず」という余裕を持たせた上限身長
    (`MAX_PLAUSIBLE_PERSON_HEIGHT_M`=2.0m)を使い、床平面投影の距離がこの上限身長から
    見てもあり得ないほど遠くなってしまった場合(=レイがほぼ水平に近づくことで生じる
    ノイズ増幅が疑われる異常値)だけ、その上限相当の距離までクランプ(切り詰め)する方式に
    変更した。通常の身長の範囲内(0.9m〜1.9m程度で検証)にいる人物であれば、このクランプは
    発動せず床平面投影の結果がそのまま使われることを数値検証で確認済み(=以前のように
    身長差によって常に手前へ引き寄せられることは無くなった)。一方、床平面投影がノイズで
    大きく膨張したケース(検証: 真の距離3mに対しノイズで9mまで膨張したケース)では、
    2.0m上限のクランプにより3.53mまで引き戻され、2026-08-17に対応した「遠くの人物が
    前後に大きく揺れ動く」不具合への効果はそのまま維持されている。
  - `config.js`の`ASSUMED_PERSON_HEIGHT_M`は`MAX_PLAUSIBLE_PERSON_HEIGHT_M`に置き換えた
    (用途が「平均で引き寄せるための想定身長」から「クランプのための上限身長」に変わった
    ため)。

- **身長ベースの距離補正機能そのものを撤去(2026-08-18、上記クランプ方式修正の直後に対応)。**
  上記の「上限身長によるクランプ」方式に修正して届けたところ、「本当に少ししか動かないので
  どうにかしてほしい」というご指摘があった。詳しく伺ったところ、症状は奥行き方向だけでなく
  「前後・左右問わず、全体的に動きが小さい」こと、また修正版(バージョン08.18.2)を反映した
  後もこの症状が続いていることを確認した。この症状は、クランプ方式についてPythonで行った
  理想化シミュレーション(通常の身長範囲(0.9m〜1.9m)であればクランプは発動せず、床平面投影
  の結果は一切変化しないはず、という検証結果)と矛盾しており、実機のカメラの視野角・設置
  角度の実際の値や、YOLOの検出キーポイントの実際の精度など、シミュレーションでは再現し
  きれない実環境固有の要因が影響している可能性が高いと判断した。このAIの開発サンドボックス
  には実際のカメラ映像を使って検証する手段が無いため、三度目の数式修正を試みるのではなく、
  **身長ベースの距離補正機能(`heightHint`)そのものを`poseGeometry.js`から完全に撤去**する
  こととした。
  - `poseGeometry.js`の`analyzePerson()`から`heightHint`の生成処理を削除し、`imageToFloor()`
    も第4引数`heightHint`を受け取らない元の3引数(`imgX, imgY, roomConfig`)に戻した。
    `imageToFloor()`内部の「見かけの身長から上限距離を求めてクランプする」処理ブロックも
    削除し、床平面へのレイ投影で求めた距離(`floorDistanceM`)をそのまま採用するようにした
    (この距離は、身長ベースの機能が追加される前から存在する、部屋の対角線に基づく上限距離
    `MAX_RAY_DISTANCE_M`と、部屋の多角形による境界クランプ`clampRayToFootprint`の対象と
    なるため、2026-08-17より前に確認いただいていた「奥まできれいに判定できていた」ときと
    同じロジックに戻る)。
  - `config.js`の`MAX_PLAUSIBLE_PERSON_HEIGHT_M`は使用箇所が無くなったため削除し、その場所に
    経緯を説明するコメントのみを残した。
  - `esbuild`によるバンドル・構文チェック、`npm run build`によるビルド確認、Node.js上での
    `analyzePerson()`/`imageToFloor()`の単体実行確認(引数エラーが出ないこと・床座標が正常に
    算出されること)、およびPlaywrightによる画面表示・ゲストログインのスモークテストを実施
    済み。ただし実際のカメラ映像を使った動きの確認は、このAIの開発サンドボックス環境では
    行えないため、実機での動作確認をお願いしたい。
